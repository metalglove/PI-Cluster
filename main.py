...
def find_candidate_above_threshold(
    ...
    Returns the index of such a circle, or None if none exists.
    ...
    # (could also pick the one with max gain for better empirical results)
    return results[0][0]
...
def greedy_threshold_single(
    ...
    for _ in range(k):
        candidate_idx = find_candidate_above_threshold(
        ...
        selected.append(circles[candidate_idx])
...
def greedy_with_guesses(
    ...
    for j in range(num_guesses + 1):
        ...
        solution = greedy_threshold_single(circles, points, k, threshold, spark)
...
def main():
    ...
    solution, coverage, best_j = greedy_with_guesses(circles, points, K, EPSILON, spark)
    ...
    # Comparison with naive greedy (always pick max gain)
    print("\n" + "-" * 60)
    print("Comparison: Naive Greedy (always pick max marginal gain)...")

    naive_solution = []
    naive_remaining = set(range(len(circles)))

    for _ in range(K):
        if not naive_remaining:
            break

        sc = spark.sparkContext
        points_bc = sc.broadcast(points)
        current_bc = sc.broadcast(naive_solution)

        candidates = [(i, circles[i]) for i in naive_remaining]

        def calc_gain(item):
            idx, c = item
            return (idx, compute_marginal_gain(c, current_bc.value, points_bc.value))

        results = sc.parallelize(candidates).map(calc_gain).collect()
        best_idx, best_gain = max(results, key=lambda x: x[1])

        if best_gain == 0:
            break

        naive_solution.append(circles[best_idx])
        naive_remaining.remove(best_idx)

    naive_coverage = compute_coverage(naive_solution, points)
    print(f"  Naive greedy selected: {len(naive_solution)} circles")
    print(f"  Naive coverage: {naive_coverage} / {len(points)} ({100*naive_coverage/len(points):.1f}%)")
